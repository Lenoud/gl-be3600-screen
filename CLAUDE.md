...
- **Screen power state.** The daemon tracks `awake` plus an `off_reason` (`schedule` | `idle` | `manual`). `scheduled_off(settings)` (using `read_clock()` for weekday/hour/minute) decides if the timer window says the display should be off now; it overrides always-on. Precedence each loop: scheduled-off blanks the screen and auto-wakes when the window reopens (only `off_reason == 'schedule'` auto-wakes); otherwise the auto-lock idle timeout applies. While scheduled-off, touches are ignored so the screen stays dark until the on-window.
