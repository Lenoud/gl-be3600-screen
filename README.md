...
1. **Home** — a clean overview: title, large online device count, per-interface
   counts (`2.4G`, `5G`, `cable`), and a `HH:MM` clock in the top-right corner.
2. **Network speed** — realtime WAN download/upload rate (auto B/KB/MB units) with a
   download (green) / upload (yellow) throughput trend sparkline that fills in as the
   daemon runs.
...
- Honours the stock screen settings, read live from uci so they match exactly what the GL admin UI sets:
  ...
  - **Scheduled on/off timer** (`gl_timer.screen`): outside the configured on-window (on the selected weekdays) the screen blanks, and it auto-wakes when the window opens. During an off-window a tap lights the screen for ~20 s, then it blanks again. The schedule overrides always-on so a nightly off-window still works.
