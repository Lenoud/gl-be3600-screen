...
`src/skyris_screen_clients.lua` 绘制一个小型的在线客户端仪表盘，分为**七个可滑动的页面**。
**左滑/下滑**进入下一页，**右滑/上滑**返回上一页；屏幕左右边缘还有可点击的
**`<` / `>` 翻页按钮**（仅在该方向存在页面时显示），点击即可翻页。

1. **首页（Home）** —— 干净的总览：标题、大号在线设备数、各接口计数（`2.4G`、`5G`、`cable`），右上角显示 `HH:MM` 时钟。
2. **网速（Network speed）** —— 实时 WAN 上下行速率（自动 B/KB/MB 单位），底部带下行（绿）/ 上行（黄）吞吐趋势折线，守护进程运行后逐步累积成线。
3. **WAN** —— 上网状态：连线 up/down、协议与设备、WAN IP、网关、DNS（未连线时各字段显示 `--`）。
4. **系统（System）** —— CPU 占用、内存占用、温度、风扇转速、负载、闪存使用率、运行时长；
   CPU / 内存 / 温度超过阈值时变红告警，并在底部显示温度趋势折线（40–90 °C 区间，最新在右侧）。
5. **风扇（Fan）** —— 点档位即可设置风扇目标温度（GL `gl_fan` 的 PID 设定点，写入 uci）：
   `QUIET` 85 °C / `AUTO` 75 °C / `COOL` 65 °C / `PERF` 55 °C。目标越低风扇越早转（越凉越吵）；
   当前档高亮，表头显示实时转速 + 当前目标。`gl_fan` 的温控安全始终保留，只是改变风扇介入的时机，不会关闭散热。
6. **设备（Devices）** —— 完整客户端列表，每页 18 个、三列显示。设备名按频段配色
   （2.4G 黄、5G 青、cable 绿；表头有图例），旁边是 IP 尾号。
   设备较多时自动翻页，右上角显示 `当前页/总页数`，滑动即可翻页（例如 100 个设备 = 6 页）。
7. **菜单（Menu）** —— 美化的功能按钮，每个都带简短说明。点击按钮：
...
| 首页 | 网速 | WAN |
|------|------|-----|
| ![首页](docs/img/home.png) | ![网速](docs/img/speed.png) | ![WAN](docs/img/wan.png) |

| 系统 | 风扇 | 设备 |
|------|------|------|
| ![系统](docs/img/system.png) | ![风扇](docs/img/fan.png) | ![设备](docs/img/devices.png) |

| 菜单 |
|------|
| ![菜单](docs/img/menu.png) |
...
- 遵循原厂屏幕设置，直接从 uci 实时读取，与 GL 后台设置完全一致：
  ...
  - **定时开关屏**（`gl_timer.screen`）：在选定星期、配置的开启时段之外屏幕熄灭，时段开始时自动点亮。关屏时段内点一下屏幕会临时亮约 20 秒，之后再自动熄灭。定时优先于"常亮"，所以夜间定时关屏依然生效。
...
渲染单个页面后退出（0 = 首页，1 = 网速，2 = WAN，3 = 系统，4 = 风扇，之后是设备页，最后是菜单）—— 方便调试：
